...
const resolvers = {
  ...
  Query: {
    ...
    allAuthors: () => {
      ...
      let authorsWWithBookCount = []
      let authorsToReturn = []

      const authorWithBookCount = authors.map(a => {          
          let authorBoookCount = books.filter(b => a.name === b.author).length          
          const updatedAuthor = { ...a, bookCount: authorBoookCount }
          authorsToReturn.push(updatedAuthor)
    })      
